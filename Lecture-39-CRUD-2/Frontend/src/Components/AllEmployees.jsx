...
export default function AllEmployees() {
    ...
    // edit emp
    const [nameedit, setNameedit] = useState('test')
    const [emailedit, setEmailedit] = useState('test@email')
    const [roleedit, setRoleedit] = useState('Admin')
    ...
    // update emp
    const updateEmployyestart = async (emp) => {
        setNameedit(emp.name)
        setEmailedit(emp.email)
        setRoleedit(emp.role)
        SetIdupdated(emp._id)
    }

    const updateEmpyeesubmit = async (e) => {
        e.preventDefault();
        ...
        if (nameedit === "" || emailedit === "" || roleedit === "") {
            ...
            try {
                const response = await fetch(`http://localhost:1999/api/emp/${idupdated}`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json"
                    },
                ...
                if (response.ok) {
                    ...
                    // setEmployee([...employee, updatedEmp])
                    setEmployee(employee.map((emp) => emp._id === idupdated ? updatedEmp:emp))
                    setNameedit("")
                    setEmailedit("")
                    setRoleedit("")
                    SetIdupdated("")
                    toast.success("Empoyee updated SuccessFully")

                } else {
                    toast.error(`This error ${response.statusText}`)
                }
            } catch (error) {
                                ...
                                <div className="col-sm-6 col-xl-4" key={emp._id}>
                                    <div className="card employee-card h-100">
                                        ...
                                        <div className="card-footer bg-transparent border-0 d-flex justify-content-between px-4 pb-4">
                                            <button className="btn btn-sm btn-outline-primary" onClick={() => updateEmployyestart(emp)}>
                        ...
                        idupdated === '' ?
                            ...
                            <div className="col-lg-4 text-center">
                                <div className="card elite-form sticky-top">
                                    ...
                                    <div className="card-body p-4">
                                        <form onSubmit={updateEmpyeesubmit}>
                                            <div className="mb-3">
                                                <label className="form-label">Full Name</label>
                                                <div className="input-group">
                                                    <span className="input-group-text">
